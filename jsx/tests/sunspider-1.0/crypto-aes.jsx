...
class _Main {
     ...
     */
    function Cipher(input : Array.<number>, w : Array.<Array.<number>>) : Array.<number> {
        ...
        var output = new number[4*Nb];  // convert state to 1-d array before returning [§3.4]
    ...
    function ShiftRows(s : Array.<Array.<number>> , Nb : number) : Array.<Array.<number>> {    // shift row r of state S left by r bytes [§5.1.2]
        var t = new number[4];
        for (var r=1; r<4; r++) {
    ...
    function MixColumns(s : Array.<Array.<number>>, Nb : number) : Array.<Array.<number>> {
        // combine bytes of each col of state S [§5.1.3]
        for (var c=0; c<4; c++) {
            var a = new number[4];  // 'a' is a copy of the current column from 's'
            var b = new number[4];  // 'b' is a•{02} in GF(2^8)
            for (var i=0; i<4; i++) {
                a[i] = s[i][c];
    ...
    function AddRoundKey(state : Array.<Array.<number>>, w : Array.<Array.<number>>, rnd : number, Nb : number) : Array.<Array.<number>> {
    ...
    function KeyExpansion(key : Array.<number>) : Array.<Array.<number>> {
        // generate Key Schedule (byte-array Nr+1 x Nb) from Key [§5.2]
        var Nb = 4;            // block size (in words): no of columns in state (fixed at 4 for AES)
        var Nk = key.length/4; // key length (in words): 4/6/8 for 128/192/256-bit keys
        var Nr = Nk + 6;       // no of rounds: 10/12/14 for 128/192/256-bit keys

        var w = new number[][Nb*(Nr+1)];
        var temp = new number[4];
    ...
    function RotWord(w : Array.<number>) : Array.<number> {
    ...
    function AESEncryptCtr(plaintext : string, password : string, nBits : number) : string {
        ...
        var counterBlock = new number[blockSize];  // block size fixed at 16 bytes / 128 bits (Nb=4) for AES
        ...
        var ciphertext = new string[blockCount];  // ciphertext as array of strings
 ...
 */
    function AESDecryptCtr(ciphertext : string, password : string, nBits : number) : string {
        ...
        var ciphertextArray = ciphertext.split('-');  // split ciphertext into array of block-length strings

        // recover nonce from 1st element of ciphertext
        var blockSize = 16;  // block size fixed at 16 bytes / 128 bits (Nb=4) for AES
        var counterBlock = new number[blockSize];
        var ctrTxt = this.unescCtrlChars(ciphertextArray[0]);
        for (var i=0; i<8; i++) counterBlock[i] = ctrTxt.charCodeAt(i);

        var plaintext = new string[ciphertextArray.length-1];

        for (var b=1; b<ciphertextArray.length; b++) {
            // set counter (block #) in last 8 bytes of counter block (leaving nonce in 1st 8 bytes)
            ...
            ciphertextArray[b] = this.unescCtrlChars(ciphertextArray[b]);

            var pt = '';
            for (var i=0; i<ciphertextArray[b].length; i++) {
                // -- xor plaintext with ciphered counter byte-by-byte --
                var ciphertextByte = ciphertextArray[b].charCodeAt(i);
    ...
    function escCtrlChars(str : string) : string {
        ...
        return str.replace(/[\0\t\n\v\f\r\xa0'"!-]/g, function(c : string) : string { return '!' + c.charCodeAt(0).toString() + '!'; });
}  // \xa0 to cater for bug in Firefox; include '-' to leave it free for use as a block marker
// "])}

    function unescCtrlChars(str : string) : string {
        // unescape potentially problematic control characters
        return str.replace(/!\d\d?\d?!/g, function(c : string) : string { return String.fromCharCode(Number.parseInt(c.slice(1,-1))); });
    }
    /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -  */
     ...
     */
    static const b64 = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=";

    function encodeBase64(str : string) : string {
        // http://tools.ietf.org/html/rfc4648
        var o1, o2, o3, h1, h2, h3, h4, bits, i=0, enc='';

        str = this.encodeUTF8(str);  // encode multi-byte chars into UTF-8 for byte-array
    ...
    function decodeBase64(str : string) : string {
        ...
        return this.decodeUTF8(enc);  // decode UTF-8 byte-array back to Unicode
    }
    ...
    function byteArrayToHexStr(b : Array.<number>) : string {
    ...
    function constructor() {
        ...
        [ "ROMEO: But, soft! what light through yonder window breaks?\n",
            ...
            "And sails upon the bosom of the air."].join("");

        var password = "O Romeo, Romeo! wherefore art thou Romeo?";

        var cipherText = this.AESEncryptCtr(plainText, password, 256);
        var decryptedText = this.AESDecryptCtr(cipherText, password, 256);
        log decryptedText;
    }
