...
class _Main {
    ...
    function toBase64(data : string) : string {
        ...
        for (i = 0; i < (length - 2); i += 3) {
            result += this.toBase64Table.charAt(data.charCodeAt(i) >> 2);
            result += this.toBase64Table.charAt(((data.charCodeAt(i) & 0x03) << 4) + (data.charCodeAt(i+1) >> 4));
            result += this.toBase64Table.charAt(((data.charCodeAt(i+1) & 0x0f) << 2) + (data.charCodeAt(i+2) >> 6));
            result += this.toBase64Table.charAt(data.charCodeAt(i+2) & 0x3f);
        }
        ...
        if (length%3 > 0) {
            ...
            if ((length%3) == 2) {
                result += this.toBase64Table.charAt(((data.charCodeAt(i) & 0x03) << 4) + (data.charCodeAt(i+1) >> 4));
                result += this.toBase64Table.charAt((data.charCodeAt(i+1) & 0x0f) << 2);
                result += this.base64Pad;
            } else {
                result += this.toBase64Table.charAt((data.charCodeAt(i) & 0x03) << 4);
    ...
    function constructor() {
        ...
        for ( var i = 8192; i <= 16384; i *= 2 ) {
            ...
            base64 = this.toBase64(str);
            this.base64ToString(base64);
